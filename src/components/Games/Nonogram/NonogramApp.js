...
export default function NonogramApp() {
  ...
  function randomValueSolution() {
    changeSolutionGame({
      grid: solutionGame.grid.map((row) =>
        row.map((item) => (item = Math.round(Math.random())))
      ),
    });
  }
  useEffect(() => {
  ...
  let changeCellValue = (x, y) => {
    ...
    objeto.grid[x][y] = objeto.grid[x][y] === 2 ? 0 : objeto.grid[x][y] + 1;
  ...
  let compareSolution = () => {
    let solutionUserWithout2 = solutionUser.grid.map((row) =>
      row.map((item) => (item !== 1 ? 0 : 1))
    );
    if (
      JSON.stringify(solutionGame.grid) === JSON.stringify(solutionUserWithout2)
    ) {
      //Calculate score
      setScore(level * 5);
  ...
  return (
    <div style={{ backgroundColor: "rgb(248, 248, 245)" }}>
      <div className="Nonogram container-fluid">
        ...
        <div className="row nonogramContent">
          {modal ? (
            <div>
              <Modal isOpen={modal}>
                ...
                <ModalFooter>
                  <Button color="primary" onClick={() => changeLevel(5)}>
                    Vamos allá
                  </Button>{" "}
                </ModalFooter>
              </Modal>
                ...
                <tbody>
                  <tr>
                    ...
                    {verticalClues.map((clue, clueIndex) => {
                      return (
                        <td key={clueIndex} valign="bottom">
                          <div className="clue v_clue">
                            <p className="v_clue_p v-text">{clue}</p>
                          </div>
                        </td>
                      );
                    })}
                  ...
                  {solutionUser.grid.map((row, rowIndex) => (
                    <tr key={rowIndex}>
                      <td>
                        <div className="clue h_clue">
                          {horizontalClues[rowIndex]}
                        </div>
                      </td>
                      {row.map((cell, cellIndex) => (
                        <td
                          key={cellIndex}
                          onClick={() => changeCellValue(rowIndex, cellIndex)}
                        >
                          <div className={`cell_${cell} cell`}></div>
                        </td>
                      ))}
                    </tr>
                  ))}
              ...
              <div className="buttons">
                ...
                <Dropdown isOpen={dropdownOpen} toggle={toggleDropdown}>
                  ...
                  <DropdownMenu>
                    <DropdownItem onClick={() => changeLevel(3)}>
                      Aprende 3x3
                    </DropdownItem>
                    <DropdownItem divider />
                    <DropdownItem onClick={() => changeLevel(5)}>
                      Fácil 5x5
                    </DropdownItem>
                    <DropdownItem divider />
                    <DropdownItem onClick={() => changeLevel(7)}>
                      Medio 7x7
                    </DropdownItem>
                    <DropdownItem divider />
                    <DropdownItem onClick={() => changeLevel(8)}>
                      Difícil 8x8
                    </DropdownItem>
                  </DropdownMenu>
                </Dropdown>
              ...
              <div className="buttons">
                ...
                <Dropdown isOpen={dropdownOpen} toggle={toggleDropdown}>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownItem divider />
                    <DropdownItem onClick={() => changeLevel(6)}>
                      Medio 6x6
                    </DropdownItem>
                    <DropdownItem divider />
                    <DropdownItem onClick={() => changeLevel(7)}>
